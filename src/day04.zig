...
pub fn main() !void {
    ...
    const results = try day04(debugAllocator.allocator(), data);

    print("Answer = {}", .{results});
}

fn day04(allocator: Allocator, input: []const u8) !struct { usize, usize } {
    var arenaAllocator: std.heap.ArenaAllocator = .init(allocator);
    defer arenaAllocator.deinit();
    const arena = arenaAllocator.allocator();

    var grid: Grid = .init(arena);
    defer {
        for (grid.items) |row| arena.free(row);
        grid.deinit();
    }

    try loadGrid(arena, input, &grid);

    var removeable: List([2]i32) = .init(arena);
    defer {
        removeable.deinit();
    }
...
fn loadGrid(allocator: Allocator, input: []const u8, grid: *Grid) !void {
...
test "Sample data" {
    ...
    try expectEqual(.{ 13, 43 }, try day04(allocator, sampleData));
}

test "real data" {
    const allocator = std.testing.allocator;

    try expectEqual(.{ 1553, 8442 }, try day04(allocator, data));
}
