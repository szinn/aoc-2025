...
pub fn main() !void {
    ...
    const results = try day07(debugAllocator.allocator(), data);

    print("Answer: {}\n", .{results});
}

fn day07(allocator: Allocator, input: []const u8) !struct { usize, usize } {
    var arenaAllocator: std.heap.ArenaAllocator = .init(allocator);
    defer arenaAllocator.deinit();
    const arena = arenaAllocator.allocator();

    var lines: List([]const u8) = .init(arena);
    var routeCounts: List([]usize) = .init(arena);
    var beams: []u8 = undefined;
    var nextBeams: []u8 = undefined;
    var sIndex: usize = undefined;
    defer {
        for (routeCounts.items) |line| arena.free(line);
        routeCounts.deinit();
        lines.deinit();
        arena.free(beams);
        arena.free(nextBeams);
    }
    ...
    while (lineIterator.next()) |line| {
        ...
        const routes = try arena.alloc(usize, line.len);
        ...
        if (lines.items.len == 1) {
            if (indexOf(u8, line, 'S')) |startIndex| {
                sIndex = startIndex;
                beams = try arena.alloc(u8, line.len);
                nextBeams = try arena.alloc(u8, line.len);
...
test "Sample data" {
    ...
    try expectEqual(.{ 21, 40 }, day07(allocator, sampleData));
}

test "real data" {
    const allocator = std.testing.allocator;

    try expectEqual(.{ 1667, 62943905501815 }, try day07(allocator, data));
}
