...
fn day02(input: []const u8) !struct { stage1: usize, stage2: usize } {
    ...
    while (rangeIterator.next()) |range| {
        ...
        if (indexOf(u8, trimmedRange, '-')) |index| {
            ...
            sum_stage1 += sumInvalidStage1Ids(lower, upper);
            sum_stage2 += sumInvalidStage2Ids(lower, upper);
        }
    }
...
fn sumInvalidStage1Ids(lower: usize, upper: usize) usize {
    var sum: usize = 0;

    for (lower..upper + 1) |i| {
        if (!isValidStage1Id(i)) {
            sum += i;
        }
    }
    return sum;
...
fn sumInvalidStage2Ids(lower: usize, upper: usize) usize {
    var sum: usize = 0;

    for (lower..upper + 1) |i| {
        if (!isValidStage2Id(i)) {
            sum += i;
        }
    }
    return sum;
