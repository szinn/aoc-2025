...
pub fn main() !void {
    ...
    const results = try day06(debugAllocator.allocator(), data);

    print("Answer: {}\n", .{results});
}

fn day06(allocator: Allocator, input: []const u8) !struct { usize, usize } {
    var arenaAllocator: std.heap.ArenaAllocator = .init(allocator);
    defer arenaAllocator.deinit();
    const arena = arenaAllocator.allocator();

    var lines: List([]const u8) = .init(arena);
    var operators: List(Operator) = .init(arena);
    defer {
        lines.deinit();
        operators.deinit();
    }
...
test "Sample data" {
    ...
    try expectEqual(.{ 4277556, 3263827 }, day06(allocator, sampleData));
}

test "real data" {
    const allocator = std.testing.allocator;

    try expectEqual(.{ 4449991244405, 9348430857627 }, try day06(allocator, data));
}
