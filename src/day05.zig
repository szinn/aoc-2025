...
pub fn main() !void {
    ...
    const results = try day05(debugAllocator.allocator(), data);

    print("Answer = {}", .{results});
}

fn day05(allocator: Allocator, input: []const u8) !struct { usize, usize } {
    var arenaAllocator: std.heap.ArenaAllocator = .init(allocator);
    defer arenaAllocator.deinit();
    const arena = arenaAllocator.allocator();

    var ranges: List(Range) = .init(arena);
    defer {
        ranges.deinit();
    }
...
test "Sample data" {
    ...
    try expectEqual(.{ 3, 14 }, try day05(allocator, sampleData));
}

test "real data" {
    const allocator = std.testing.allocator;

    try expectEqual(.{ 758, 343143696885053 }, try day05(allocator, data));
}
